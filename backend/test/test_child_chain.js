...
async function createTx(utxo, account, to) {
  ...
  ///onsole.log(txData.signature);
  let createdTx = new PlasmaTransaction(txData);
  //console.log(keys[account.toLowerCase()]);
  createdTx.sign(keys[account.toLowerCase()]);

  //console.log(createdTx.signature);
...
describe('ChildChain', function () {
  ...
  describe('Check Block Creation', async function () {
    ...
    it('should create transactions from utxos and write block', async function () {
      ...
      console.log('      time for 1000 txs ', (t1 - t0)/1000*10, ' s' )
      console.log('      time for 5000 txs ', (t1 - t0)/1000*50, ' s' )
