...
describe("CHANGING OWNER", () => {
  it(`Should change token owner: from ${ACCOUNTS[0].address} to ${ACCOUNTS[1].address}`, async () => {
    ...
    console.log(`2. Answer for "getTokenByAddress" and address ${oldAcc.address}:`, answer);
    ...
    console.log('====> Token to use for test changing:', token.id);
    expect(token.id).to.be.string;

    const lastTx = await plasma({action: "getLastTransactionByTokenId", payload: {tokenId: token.id}});
    console.log(`3. Last transaction of ${oldAcc.address} by token ${token.id}:`, lastTx);
    ...
    console.log('4. Data for transaction:', data);
    ...
    console.log('5. Data to encode:', dataToEncode);
    ...
    console.log('6. Transaction has been sent, answer:', sentTx);
